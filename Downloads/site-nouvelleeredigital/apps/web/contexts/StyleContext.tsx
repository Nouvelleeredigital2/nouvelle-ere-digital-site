...
export const StyleProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  useEffect(() => {
    ...
    document.documentElement.style.setProperty('--border-radius', getBorderRadiusValue(config.borderRadius));
    document.documentElement.style.setProperty('--spacing', getSpacingValue(config.spacing));
    document.documentElement.style.setProperty('--bg-color', config.mode === 'dark' ? '#1a1a1a' : '#ffffff');
    document.documentElement.style.setProperty('--text-color', config.mode === 'dark' ? '#ffffff' : '#000000');

    // Appliquer la police globale
    document.body.style.fontFamily = config.bodyFont === 'playfair' ? 'Playfair Display, serif' :
