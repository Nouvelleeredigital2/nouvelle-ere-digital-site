...
export const TrajectoireIAOverlay: React.FC<TrajectoireIAOverlayProps> = ({
  ...
  return (
    ...
    >
      ...
      {onAnimate && (
        <button onClick={onAnimate} className="absolute top-2 left-2 bg-[var(--color-primary)] text-white px-2 py-1 rounded">
          Animer
        </button>
      )}
      {onReset && (
        <button onClick={onReset} className="absolute top-2 right-2 bg-red-500 text-white px-2 py-1 rounded">
          Réinitialiser
        </button>
      )}
    </div>
