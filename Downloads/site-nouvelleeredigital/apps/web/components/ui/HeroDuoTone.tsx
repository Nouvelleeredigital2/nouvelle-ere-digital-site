  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          'absolute inset-0',
          variant === 'overlay' && `bg-gradient-to-b from-[${primaryColor}] to-[${secondaryColor}] opacity-50`,
          variant === 'filter' && `filter hue-rotate-90 saturate-150`
        )}
      />
      <button onClick={onToggle} className="absolute top-4 right-4 bg-[var(--color-primary)] text-white px-3 py-1 rounded">
        Toggle
      </button>
    </div>
  );
