...
export function Universe3D({
  ...
  const [polePositions, setPolePositions] = useState<Array<{
    x: number;
    y: number;
    visible: boolean;
    service: ServicePole;
  ...
  const polePositionsRef = useRef<Array<{
    x: number;
    y: number;
    visible: boolean;
    service: ServicePole;
    ...
    try {
      ...
      services.forEach((service) => {
        ...
        poleObjects.push({
          ...
          halos: poleGroup.children.filter((c: any) => c.geometry?.type === 'SphereGeometry' && c !== sphere),
          data: service,
          currentScale: 1
        });
      });
      ...
      const animate = () => {
        ...
        poleObjects.forEach((poleObj, index) => {
          ...
          for (let i = 0; i < positions.length; i += 3) {
            ...
            const currentPhi = Math.acos(Math.max(-1, Math.min(1, positions[i + 2] / vel.r))) + vel.phi;
    ...
    <div className={`relative w-full h-screen bg-black ${className}`}>
      ...
      >
        <div id="instructions" className="sr-only">
          Utilisez les flèches pour faire tourner l&apos;univers. 
          Tab pour naviguer entre les services. 
                ...
                pos.visible && (
                  ...
                  >
                    ...
                    {hoveredPole === pos.service && (
                      <div className="absolute left-0 top-6 w-2 h-2 rounded-full animate-ping" 
